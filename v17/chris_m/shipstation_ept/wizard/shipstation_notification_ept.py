...
class ShipstationNotificationEpt(models.TransientModel):
    """
    Describes Shipstation Instance Configurations
    """
    _name = 'shipstation.notification.ept'
    ...
    user_ids = fields.Many2many('res.users', 'shipstation_notification_user_ids_rel',
    ...
    def set_notification(self):
        instance = self.env['shipstation.instance.ept'].browse(self.env.context.get('active_id', 0))
        if instance:
            instance.write({
    ...
    def default_get(self, fields):
        result = super(ShipstationNotificationEpt, self).default_get(fields)
        instance = self.env['shipstation.instance.ept'].browse(self.env.context.get('active_id', 0))
        if instance:
            result.update({
