...
class ShipstationInstanceEpt(models.Model):
    ...
    activity_user_ids = fields.Many2many(comodel_name='res.users',
                                         ...
                                         help="Schedule activity will be created against this users.")
    activity_lead_days = fields.Integer(string="Lead Days",
                                        help="Deadline of the created activity")
    ...
    def _prepare_activity_vals(self,**kwargs):
        instance = kwargs.get('instance')
        activity_type = instance.activity_type
        activity_vals = {}
        if activity_type:
            activity_vals.update({'res_name': 'Delivery to Validate',
                           'activity_type_id': activity_type.id,
                           'date_deadline': datetime.now() + relativedelta(days=instance.activity_lead_days),
                           'res_id': kwargs.get('picking').id,
                           'user_id': kwargs.get('user').id,
                           'summary': "Please take required action to validate this delivery order",
