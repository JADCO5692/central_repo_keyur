...
class SignRequest(models.Model):
    ...
    def _send_completed_document_mail(self, signers, request_edited, partner, access_token=None, with_message_cc=True, force_send=False):
        ...
        self.ensure_one()
        emails = [user['email'] for user in signers if 'id' in user]
        if partner.npc_user_type != "APP" or not partner.email in emails:
            super(SignRequest,self)._send_completed_document_mail(signers, request_edited, partner, access_token, with_message_cc, force_send)
        else:
    ...
    def check_signers(self,user_data_list):
        """
        Checks if any user in the provided list has the user_type 'np'. 
        """ 
        user_ids = [user['email'] for user in user_data_list if 'id' in user]
        if not user_ids:
            return False
        np_user_count = self.env['res.partner'].search_count([('email', 'in', user_ids),('npc_user_type', '=', 'APP')]) 
        return np_user_count > 0
