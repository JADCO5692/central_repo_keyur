...
class SaleCommissionLogs(models.Model):
    ...
    vendor_bill_ids = fields.Many2many('account.move', 'commission_logs_account_move_rel', 'commission_log_id',
                                       'account_move_id', string='Vendor Bills')
                ...
                if sale_order or invoice.custom_lead_id2:
                    ...
                    for i, colabs in enumerate(collaborators):
                        ...
                        if first_invoice and first_invoice.id != invoice.id:
                            first_month_physician_fee, first_month_npc_fee = self.get_first_commission_fees(
                                first_invoice, colabs)
                        else:
                        ...
                        log_values = {
                            ...
                            'npc_fee': npc_fee,
                            'prorate_percentage': round((physician_fee / physician_monthly_fee),
                                                        2) if physician_monthly_fee > 0 else 0,
    ...
    def action_create_vendor_bill(self):
        ...
        for log in self:
            product = Product.search([('is_physician_main_fees_product', '=', True)], limit=1)
            if not product or not log.collaborator_id:
                continue
            bill_vals = {
                'move_type': 'in_invoice',
                'partner_id': log.collaborator_id.id,
                'ref': log.invoice_id.name,
                'invoice_date': log.invoice_id.invoice_date_due,
                'invoice_date_due': log.invoice_id.invoice_date_due,
                'related_commission_id': log.id,
                'date': log.invoice_id.invoice_date_due,
                'invoice_origin': f'Bill From {log.name}',
                'invoice_line_ids': [(0, 0, {
                    'product_id': product.id,
                    'quantity': 1,
                    'price_unit': log.physician_fee,
                    'name': product.name,
                })],
            }
            bill = AccountMove.create(bill_vals)
            log.invoice_id.vendor_bill_ids = [(4, bill.id)]
            log.vendor_bill_ids = [(4, bill.id)]
            bill.action_post()
